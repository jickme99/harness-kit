...
- **Self-updating is PROPOSAL-ONLY.** Projects poll the kit (`CHANGELOG.md` vs their
  ...
  Recipe: `spec/versioning.md`. Kit-opens-PRs is later automation (consumer #2). The
  stamp that makes a poll possible — `kit-manifest.json`, generated by
  `reference/tools/kit_manifest.py` — ships now, in every stamp. Current kit version is
  in that file and in `CHANGELOG.md`.
