...
- **Self-updating is PROPOSAL-ONLY.** Projects poll the kit (`CHANGELOG.md` vs their
  ...
  Tool: `reference/tools/kit_poll.py`. Recipe: `spec/versioning.md`. Kit-opens-PRs is
  later automation (consumer #2). The stamp that makes a poll possible —
  `kit-manifest.json`, generated by `reference/tools/kit_manifest.py` — ships now, in
  every stamp. Current kit version is in that file and in `CHANGELOG.md`.
...
| `reference/tools/` | `kit_manifest.py` (upgrade classifier) · `kit_poll.py` (changelog worklist + STANDUP mapping; never copies files) · `review_stamp.py` (config stamper) · `review_rates.py` (catch/escape rollup). |
